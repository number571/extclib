...
void aes_key_setup(const uint8_t key[],          // The key, must be 128, 192, or 256 bits
                   ...
                   uint16_t keysize);              // Bit length of the key, 128, 192, or 256
...
int aes_decrypt_cbc(const uint8_t in[],          // Ciphertext
                    size_t in_len,            // Must be a multiple of AES_BLOCK_SIZE
                    uint8_t out[],               // Plaintext, same length as plaintext
                    const uint32_t key[],         // From the key setup
                    uint16_t keysize,              // Bit length of the key, 128, 192, or 256
                    const uint8_t iv[]);         // IV, must be AES_BLOCK_SIZE bytes long
...
int aes_encrypt_ccm(const uint8_t plaintext[],              // IN  - Plaintext.
                    ...
                    uint16_t associated_data_len,  // IN  - Associated Data length in bytes.
                    ...
                    uint16_t keysize);                        // IN  - The length of the key in bits. Valid values are 128, 192, 256.
